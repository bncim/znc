...
void CClient::SendRequiredPasswordNotice() {
	PutClient(":irc.bnc.im 464 " + GetNick() + " :Password required");
	PutClient(":irc.bnc.im NOTICE AUTH :*** "
...
void CClient::ReadLine(const CString& sData) {
	...
	if (sCommand.Equals("ZNC")) {
		...
		if (sLine.length() >= 5)
			PutClient(":irc.bnc.im PONG irc.bnc.im " + sLine.substr(5));
		else
			PutClient(":irc.bnc.im PONG irc.bnc.im");
...
void CClient::RefuseLogin(const CString& sReason) {
	...
	PutClient(":irc.bnc.im 464 " + GetNick() + " :" + sReason);
...
void CClient::PutModNotice(const CString& sModule, const CString& sLine) {
	...
	DEBUG("(" << GetFullName() << ") ZNC -> CLI [:" + m_pUser->GetStatusPrefix() + ((sModule.empty()) ? "status" : sModule) + "!znc@bnc.im NOTICE " << GetNick() << " :" << sLine << "]");
	Write(":" + m_pUser->GetStatusPrefix() + ((sModule.empty()) ? "status" : sModule) + "!znc@bnc.im NOTICE " + GetNick() + " :" + sLine + "\r\n");
}
...
void CClient::PutModule(const CString& sModule, const CString& sLine) {
	...
	DEBUG("(" << GetFullName() << ") ZNC -> CLI [:" + m_pUser->GetStatusPrefix() + ((sModule.empty()) ? "status" : sModule) + "!znc@bnc.im PRIVMSG " << GetNick() << " :" << sLine << "]");

	sLine.Split("\n", vsLines);
	for (it = vsLines.begin(); it != vsLines.end(); ++it) {
		Write(":" + m_pUser->GetStatusPrefix() + ((sModule.empty()) ? "status" : sModule) + "!znc@bnc.im PRIVMSG " + GetNick() + " :" + (*it) + "\r\n");
	}
}
...
{
	PutClient(":irc.bnc.im CAP " + GetNick() + " " + sResponse);
}
...
void CClient::HandleCap(const CString& sLine)
{
	...
	if (sSubCmd.Equals("LS")) {
		...
		PutClient(":irc.bnc.im 410 " + GetNick() + " " + sSubCmd + " :Invalid CAP subcommand");
	}
}
